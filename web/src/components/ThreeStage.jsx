...
const ThreeStage = ({
  ...
  useEffect(() => {
    ...
    if (showFrame) {
      ...
      railGeometry = new BoxGeometry(beltLength, railThickness, railThickness);
      ...
      leftRail.position.set(0, beltWidth / 2 + railThickness / 2, railZOffset);
    ...
    const createItemsWithInitialTextures = () => {
      ...
      const count = Math.min(manifestUrls.length || visibleCount, visibleCount);
        ...
        if (!isPaused && pauseOnCenter) {
          for (let i = 0; i < itemsArr.length; i++) {
            ...
            if (crossed && Math.abs(currX) <= centerThreshold) {
              ...
              if (zoomOnPause) {
                const tex =
                  mesh.material && mesh.material.map ? mesh.material.map : null;
                if (tex) {
